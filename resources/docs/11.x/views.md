...
Представлення відокремлюють логіку вашого контролера/додатку від логіки вашого представлення і зберігаються в каталозі `resources/views`. При використанні Laravel, шаблони представлень зазвичай пишуться за допомогою [Мова шаблонів blade](/docs/{{version}}/blade). Простий погляд може виглядати приблизно так:
...
Як ви можете бачити, перший аргумент, переданий допоміжному засобу `view`, відповідає імені файлу подання у каталозі `resources/views`. Другий аргумент - це масив даних, які слід зробити доступними для подання. У цьому випадку ми передаємо змінну `name`, яка відображається у поданні за допомогою [Синтаксис blade](/docs/{{version}}/blade).
